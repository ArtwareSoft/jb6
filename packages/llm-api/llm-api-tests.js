  ...
  impl: dataTest({
    calculate: rx.pipe(
      source.llmCompletions({
        prompt: user(`You are an expert medical assistant for doctors in emergency settings. 
    Given this brief description of a patient or their symptoms, 
    Query: age 40, dizziness, stomach ache
    context: Balance issues
    generate a JSON list of *5* diagnostic suggestions. 
    ...
    **Response format (JSON)**
    ...
    `),
        llmModel: gpt_35_turbo_0125(),
        notifyUsage: ctx => console.log(`cost: ${ctx.data.totalCost}`)
      }),
      llm.textToJsonItems(),
