...
import { testUtils } from '../testers/tester.js'
import { spy } from '@jb6/testers/spy.js'

const { Ctx, jb, globalsOfType, asJbComp, unique, delay } = coreUtils
const { 
    test : { Test }
} = dsls  
...
export async function runTests({specificTest,show,pattern,notPattern,take,remoteTests,repo,onlyTest,top,coveredTestsOf,showOnly}={}) {
    ...
    tests.reduce(async (pr,{testID}) => {
        ...
        let res
        if (!showOnly) {
            if (globalThis.document) document.getElementById('progress').innerHTML = `<div id=${testID}>${index++}: ${testID} started</div>`
            console.log('starting ' + testID )
            res = await testUtils.runTest(testID,{fullTestId, singleTest })
            console.log('end      ' + testID, res)
            if (globalThis.document) document.getElementById('progress').innerHTML = `<div id=${testID}>${testID} finished</div>`
