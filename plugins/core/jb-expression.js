...
export function calc(_exp, ctx, args, tgpCtx) {
    const {parentParam} = tgpCtx
    ...
    if (exp.indexOf('$log:') == 0) {
      const out = calc(exp.split('$log:')[1],ctx,parentParam)
      console.log(out, ctx, tgpCtx)
      return out
    }
    ...
    function expPart(expressionPart,_parentParam) {
      return resolveFinishedPromise(evalExpressionPart(expressionPart,ctx,args, _parentParam || parentParam))
    }
...
function evalExpressionPart(expressionPart,ctx,args, parentParam) {
    const jstype = parentParam && (parentParam.ref ? 'ref' : parentParam.as)
    // example: %$person.name%.
    ...
    function pipe(input,subExp,last,first,invokeFunc) {
      ...
      if (first && subExp.charAt(0) == '$' && subExp.length > 1) {
        const ret = calcVar(subExp.substr(1), {ctx, args, jstype: last ? jstype : null})
...
export function calcBool(exp, ctx, args, tgpCtx) {
    if (exp.indexOf('$debugger:') == 0) {
      debugger
      exp = exp.split('$debugger:')[1]
    }
    if (exp.indexOf('$log:') == 0) {
      ...
      const result = calcBool(exp.split('$log:')[1], ctx, args, tgpCtx)
      console.log(result, calculated, ctx, args, tgpCtx)
      return result
    }
    if (exp.indexOf('!') == 0)
      return !calcBool(exp.substring(1), ctx, args, tgpCtx)
    const parts = exp.match(/(.+)(==|!=|<|>|>=|<=|\^=|\$=)(.+)/)
    if (!parts) {
      const ref = calc(exp, ctx, args, tgpCtx)
      if (isRef(ref))
        return ref
    ...
    if (parts.length != 4)
      return logError('invalid boolean expression: ' + exp, {ctx, tgpCtx})
    const op = parts[2].trim()
  
    if (op == '==' || op == '!=' || op == '$=' || op == '^=') {
      let [p1, p2] = [doCalcString(parts[1]), doCalcString(parts[3])]
      p2 = tostring(calc(trim(parts[3]), ctx, args, { parentParam: {as: 'string'} } ))
    ...
    function doCalcString(exp) {
        return tostring(calc(trim(exp), ctx, args, { parentParam: {as: 'string'} } ))
    }
    function doCalcNumber(exp) {
        return tonumber(calc(trim(exp), ctx, args, tgpCtx))
    }
}
